# based on https://en.wikipedia.org/wiki/List_of_mathematical_constants
# wherever the symbols dont clash

# latin -----------------------------------------------------------------------

C = 0.98943127383114695174

# euler's number
E = 2.71828182845904523536

# imaginary unit
I = 1j

# komornik-loreti constant
Q = 1.78723165018296593301

# catalan's constant
CATALAN = 0.9159655941772190150

# MRB constant
S = 0.18785964246206712024

# first nielsen–ramanujan constant
A_1 = 0.82246703342411321823

# brun's constant
B_2 = 1.902160583104

# greek -----------------------------------------------------------------------

# euler-mascheroni constant
GAMMA = 0.57721566490153286060

# golden ratio
# (1 + √5) / 2
PHI = 1.61803398874989484820458683436563811772030917980576286213544862270526046281890244970720720418939113748475
#ϕ = 1.61803398874989484820

# silver ratio
# 1 + √2
SIGMA = 2.41421356237309504880

# archimedes constant
PI = 3.14159265358979323846264338327950288419

# lemniscate constant
#ϖ = 2.62205755429211981046

TAU = 6.28318530717958647692

# supergolden ratio
ψ = 1.46557123187676802665


# -----------------------------------------------------------------------------

