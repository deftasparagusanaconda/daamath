...
class DomainViolation(SignatureViolation):
    ...
    def __init__(self, function_name: str, args: tuple[Any, ...], kwargs: dict[str, Any], offender: Any, offender_domain: Callable):
        args: list[str] = [f'{type(input).__name__} {arg}' for arg in args]
        kwargs: list[str] = [f'{key}={type(val).__name__} {val!r}' for key, val in kwargs.items()]
        
        inputs: str = ', '.join(args + kwargs)

        super().__init__(f'{type(self).__name__}: {function_name}({inputs}) received {type(offender)} {offender!r} which is not in {offender_domain.__name__}')
...
class CodomainViolation(SignatureViolation):
    ...
    def __init__(self, function_name: str, args: tuple[Any, ...], kwargs: dict[str, Any], offender: Any, offender_domain: Callable):
        ...
        super().__init__(f'{type(self).__name__}: {function_name}({inputs}) tried to return {type(offender)} {offender!r} which is not in {offender_domain.__name__}')
